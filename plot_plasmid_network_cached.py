#!/usr/bin/env python3
"""
Plot a plasmid similarity network from nucmer all_coords.tsv using the SAME unbiased
edge logic as your core/accessory script (no overcounting overlapping rows):

Edge metric per pair (u, v):
  - For each side separately (u and v), greedily de-overlap aligned intervals
    preferring higher-identity rows first; compute:
        union_len(side), id_len_sum(side), piece_count(side)
  - shared_bp = min(union_len(u), union_len(v))
  - mean_id   = id_len_sum(side_with_min_union) / union_len(side_with_min_union)
  - Keep edge if shared_bp >= --edge-min-length

Features:
  1) Caching: first pass computes edges/nodes and writes JSON cache.
  2) Layout sweeps: try -n different seeds and save a figure for each.
  3) Coloring: --color-sources any of {group,gene,block}.
     - Groups from --groups-file group_report.tsv (group, size, members).
     - Gene label from FASTA filename ("bla..." convention, first token).
     - Blocks from --blocks-file blocks_annotated.tsv with --block-id repeated.
     Nodes with multiple labels are drawn as pie slices (up to 6).
  4) Hide small isolated components: drop components with size <= K.
  5) Robust name matching: --name-match-mode {stem,basename,exact}; --debug-dump-names.
"""

from __future__ import annotations
import argparse, csv, hashlib, json, logging, random
...
def norm_stem(name: str) -> str:
    # strips directories and one extension (.fa/.fna/.fasta etc.)
...
def parse_genes_from_name(filename: str) -> List[str]:
    """Simple parser: from basename stem, take text after 'bla' and split on 'and'."""
...
def merge_union_weighted(intervals_with_id: List[Tuple[int,int,float]]) -> Tuple[int, float, int]:
    """
    De-overlap with identity preference:
    fill uncovered bp with higher-identity rows first.
    Return (union_len, id_len_sum, pieces).
    """
    if not intervals_with_id:
        return 0, 0.0, 0
...
def read_coords_build_edges(coords_path: Path, min_identity: float, edge_min_len: int,
    ...
    """
    Read all_coords.tsv. Build per-pair side intervals with identity,
    then construct edges via the unbiased union logic above.
    """
    ...
    with coords_path.open() as fh:
        ...
        for row in rdr:
            ...
            if ident < min_identity:
                continue
            kept += 1
            ref = name_norm(row["ref_file"])
            qry = name_norm(row["query_file"])
...
def read_groups(groups_path: Path, name_norm):
    """
    Return:
      - node2groups: node -> [raw_group, ...]   (raw may be _altX)
      - group_to_canon: raw_group -> canonical group (alt collapsed)
      - canon_group_sizes: canonical group -> max(size) among its rows
    """
    ...
    with groups_path.open() as fh:
        ...
        for row in rdr:
            ...
            if size > canon_group_sizes[canon]:
                canon_group_sizes[canon] = size
            members = [name_norm(m.strip()) for m in row["members"].split(",") if m.strip()]
            for m in members:
                node2groups[m].append(raw)
...
def read_blocks_have_nodes(blocks_path: Path, block_ids: List[str], name_norm: Callable[[str], str]) -> Dict[str, Set[str]]:
    ...
    out: Dict[str, Set[str]] = {bid: set() for bid in wanted}
...
def build_graph_from_cache(nodes: List[str], edges: List[dict]) -> nx.Graph:
    ...
    for e in edges:
        G.add_edge(e["u"], e["v"],
                   shared_bp=e["shared_bp"], mean_id=e["mean_id"], blocks=e["blocks"])
    return G
...
def node_label_bundles(
    ...
    gene_mode: str = "first",  # if you added this earlier
) -> Dict[str, List[str]]:
    ...
    blocks_only = (set(color_sources) == {"block"})  # only blocks requested
    ...
    for n in nodes:
        ...
        if "group" in color_sources:
            ...
            canon_grps = [group_to_canon.get(g, g) for g in raw_grps]
            canon_grps = list(dict.fromkeys(canon_grps))
            majors = [g for g in canon_grps if canon_group_sizes.get(g, 0) >= group_major_min]
            if majors:
                labs.extend(majors)
            elif canon_grps:
                labs.append("__MINOR_GROUP__")  # from your earlier tweak
        ...
        if "gene" in color_sources:
            ...
            if genes:
                if gene_mode == "all":
                    labs.extend(genes)        # use ALL genes (e.g., NDM1 and OXA181)
                else:
                    labs.append(genes[0])     # original behavior


        # --- blocks (if enabled) ---
        if "block" in color_sources and block2nodes:
            block_hits = []
            for bid, members in block2nodes.items():
                if n in members:
                    block_hits.append(bid)
            if block_hits:
                labs.extend(block_hits)
            elif blocks_only:
                # If you ALSO want a grey slice even when mixing with group/gene,
                # replace `elif blocks_only:` with just `else:`
                labs.append("__NO_BLOCK__")
...
def draw_pie_nodes(ax, pos: Dict[str, Tuple[float,float]],
    ...
    """
    Draw each node as a pie of up to k slices based on the node's labels.
    If >6 labels, collapse the tail into an '…' slice.
    """
    max_slices = 6
    for n, (x, y) in pos.items():
        ...
        r = (node_size ** 0.5) / 200.0  # radius scaled to node_size
        theta0 = 90.0
        frac = 1.0 / len(labs)
        for lab in labs:
...
def plot_network(
    ...
    if G.number_of_edges() == 0 and G.number_of_nodes() == 0:
        logging.warning("[plot] nothing to plot")
        return
    ...
    if show_labels:
        nx.draw_networkx_labels(H, pos, font_size=label_font, ax=ax)
...
def dump_name_debug(
    outdir: Path,
    nodes_from_coords: List[str],
    node2groups: Dict[str, List[str]],
    node2labels: Dict[str, List[str]],
):
    outdir.mkdir(parents=True, exist_ok=True)
    coords_set = set(nodes_from_coords)
    groups_set = set(node2groups.keys())
...
def main():
    ...
    ap.add_argument("--color-sources", default="group", help="Comma-separated subset of: group,gene,block")
    # plotting
    ...
    ap.add_argument("--name-match-mode", choices=["stem","basename","exact"], default="stem",
                    help="How to normalize names before matching between coords, groups, blocks, and genes")
    ap.add_argument("--debug-dump-names", action="store_true",
                    help="Dump name sets and mapping diagnostics to output-dir")
    # logging verbosity
    ap.add_argument("--verbose", action="store_true", help="Enable debug logging")
    ap.add_argument("--quiet", action="store_true", help="Minimal logging")
    # Fix for argparse booleans without extra plumbing
    ap.register("action", "true_const", lambda **kw: argparse._StoreConstAction(const=True, **kw))
    ap.add_argument("--gene-mode", choices=["first","all"], default="first",
                    help="When coloring by gene, use only the first gene in the filename or all genes (pie-slices)")
    ap.add_argument("--group-major-min", type=int, default=10,
                    help="Groups with >= this many members are 'major' (colored); others are grey 'minor'.")
    args = ap.parse_args()
    ...
    nodes, edges = load_or_build_cache(
        ...
        edge_min_len=args.edge_min_length,
        log_interval=args.log_interval,
        cache_path=cache_path,
        recompute=args.recompute,
        name_norm=name_norm,
        name_mode=args.name_match_mode
    )

    # 2) Build G and apply small-component filter if requested
    ...
    # 3) Load metadata: groups, genes, blocks
    ...
    node2labels = node_label_bundles(
        ...
        group_to_canon=group_to_canon,
        canon_group_sizes=canon_group_sizes,
        group_major_min=args.group_major_min,
        gene_mode=args.gene_mode,              # <- add this
    )
    ...
    if args.debug_dump_names:
        dump_name_debug(
            ...
            nodes_from_coords=nodes,
            node2groups=node2groups,
    ...
    # grey for minor groups (if used)
    if "__MINOR_GROUP__" in {lab for labs in node2labels.values() for lab in labs}:
        label2color["__MINOR_GROUP__"] = (0.75, 0.75, 0.75, 1.0)

    # grey for nodes with no selected blocks
    if "__NO_BLOCK__" in {lab for labs in node2labels.values() for lab in labs}:
        label2color["__NO_BLOCK__"] = (1.00, 1.00, 1.00, 1.0)
